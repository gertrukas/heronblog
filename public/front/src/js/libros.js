...
const obtenerColecciones = async () => {
  try {
    const res = await fetch("/src/data/colecciones.json");
    const { data } = await res.json();
    imprimirColecciones(data);
    console.log(data);
  } catch (error) {
    console.log(error);
  }
...
const imprimirColecciones = (data) => {
  // DOM HTML de las colecciones
  const colecciones = document.querySelector("#colecciones");

  // Recorrer el arreglo de colecciones
  data.forEach(({ coleccion, productos }) => {
    imprimirTags(coleccion);
    ...
    encabezadoColeccion.innerHTML = `
      ...
      <a href="/src/pages/coleccion.html?coleccion=${coleccion?.url_coleccion}">Ver más</a>
    `;
    ...
    productos.forEach((producto) => {
      const { id, url_book, nombre, precio, url_imagen, url_amazon } = producto;
...
const imprimirTags = ({ nombre, url_coleccion }) => {
  const tags = document.querySelector("#libros-tags");
  // Imprimir los tags
  tags.innerHTML += `
      <a href="/src/pages/coleccion.html?coleccion=${url_coleccion}"  title="${nombre}">
          ${nombre}
      </a>
  `;
};
