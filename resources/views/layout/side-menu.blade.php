...
@section('content')
    ...
    <div class="flex mt-[4.7rem] md:mt-0">
        <!-- BEGIN: Side Menu -->
        <nav class="side-nav">
            <a href="" class="flex items-center  justify-center rounded-md">
                {{-- <img alt="Jmliteratour" class="w-28 rounded-md" src="{{ asset('front/img/logo.png') }}"> --}}
            ...
            <ul>
                @foreach ($side_menu as $menuKey => $menu)
                    @if ($menu == 'devider')
                        ...
                        @can($menu['can'])
                        <li>
                            <a href="{{ isset($menu['route_name']) ? route($menu['route_name'], $menu['params']) : 'javascript:;' }}" class="{{ $menu['route_name_two'] == $page_name ? 'side-menu side-menu--active' : '' }}  {{ $first_level_active_index == $menuKey ? 'side-menu side-menu--active' : 'side-menu' }}">
                                <div class="side-menu__icon">
                                    <i class=" text-lg fas fa-{{ $menu['icon'] }}" ></i>
                                </div>
